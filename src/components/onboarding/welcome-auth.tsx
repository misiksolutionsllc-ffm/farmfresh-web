...
// ============================================
// WELCOME ONBOARDING — 5 photo slides
// ============================================
const slides = [
...
export function WelcomeScreen() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', background: '#fff', zIndex: 9999 }}>
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        <img src={s.image} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
      </div>
      <div style={{ padding: '24px 24px 44px', textAlign: 'center', flexShrink: 0, background: '#fff' }}>
        <h2 style={{ color: '#5a7a3a', fontSize: 19, fontWeight: 600, lineHeight: 1.35, margin: '0 0 8px', fontFamily: 'system-ui' }}>{s.title}</h2>
        <p style={{ color: '#999', fontSize: 13, lineHeight: 1.5, margin: '0 0 24px', fontFamily: 'system-ui' }}>{s.desc}</p>
        <button
          onClick={() => isLast ? completeOnboarding() : setPage(page + 1)}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '11px 28px', borderRadius: 10, background: '#6b8c42', color: '#fff', fontSize: 14, fontWeight: 500, border: 'none', cursor: 'pointer', fontFamily: 'system-ui' }}
        >
          <span style={{ display: 'flex', gap: 4 }}>
            {slides.map((_, i) => (<span key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: i === page ? '#fff' : 'rgba(255,255,255,0.35)' }} />))}
          </span>
...
const roleOptions: { id: UserRole; label: string; emoji: string }[] = [
  { id: 'customer', label: 'Consumer', emoji: '\ud83d\uded2' },
...
export function AuthScreen() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_e: any, sess: any) => {
      if (sess?.user) handleAuthSuccess(sess.user);
    });
  ...
  const handleAuthSuccess = async (user: any) => {
    try {
      ...
      useAppStore.setState({ authedEmail: user.email, authedProvider: 'email', authedRole: appRole, role: appRole });
